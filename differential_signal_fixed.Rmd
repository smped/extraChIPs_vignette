...
regions as either blacklisted or greylisted regions.
...
We'll immediately pass our black & greylisted regions to our parsing function 
...
In the following we'll prform multiple operations in a single call mutate, so 
...
Finally a PCA plot can also provide insight as to where the variaibility in the data lies.
...
```{r plot-hfgc-with-genes2, fig.cap = "*Coverage for our region showing the relationship of signal to annotated genes*"}
feat_list <- list(

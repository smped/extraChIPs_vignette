...
Please ensure you have all data in this location.
...
adding the filename to each element by default.
...
```{r plotcpm, fig.cap = "Densities for logCPM values across all samples after discarding windows less likely to contain signal from our ChIP target"}
...
Let's start by plotting the entire region contained in `results_gr` using the 

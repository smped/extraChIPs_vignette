...
Any of the methods taken by `edgeR::calacNormFactors()` can be used here.
...
```{r plot-ma-tmm, fig.cap = "MA plot using TMM normalisation. The blue line corresponds to a loess curve fitted through the points."}
tmm_gr %>% 
  as_tibble() %>% 
  ggplot(aes(logCPM, logFC)) +
  geom_point(alpha = 0.6) +
...
```{r plot-ma-mapped, fig.cap = "MA-plot with the top 12 regions by significance labelled according to the most likely gene "}
tmm_results %>% 
  as_tibble() %>% 
...
regions$upstream <- gencode$transcript %>% 
  ...
  reduceMC() %>% 
  setdiffMC(regions$promoter)
regions$exon <- gencode$exon %>% 
  select(gene_id, ends_with("name")) %>% 
  setdiffMC(unlist(regions))
